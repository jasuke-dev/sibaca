        ...
        <form action="/admin/{{ $page }}" method="POST">
            ...
            <div class="mx-8 px-8 py-4 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <label for="type" class="block mt-4 text-sm">
                    ...
                    <input type="text" class="block w-full text-sm bg-gray-100 shadow-sm dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:focus:shadow-outline-gray form-input rounded-md p-3 mt-3 appearance-none border" placeholder="Fill New {{ $title }}" name="{{ $page }}" required autofocus>
                    @error('type')
